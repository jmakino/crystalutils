...
def hybrid1(x,v,dt, accfunc, kfunc,integrator)
  along = -> (x){accfunc.call(x)*kfunc.call(sqrt(x*x))}
  ashort = -> (x){accfunc.call(x)*(1.0-kfunc.call(sqrt(x*x)))}
  ah1 = along.call(x)
  v = v+dt*0.5*ah1
  nsteps = 128
  dt2 = dt/nsteps
...
printf("%25.20e %25.20e %25.20e %25.20e %25.20e %25.20e %d\n", 0,
       x[0],x[1], v[0],v[1], 0,0);
kfunc= method(:k2)
ifunc= method(:leapfrog_1step)
if type > 1
  kfunc= method([:k2, :k1, :k][(type-2)%3])
  ifunc = method(:yoshida4) if type>4
end

n.times{|i|
  if type==0	
    x,v = leapfrog_1step(x,v,dtreal,method(:acc))
  elsif type==1
    x,v = yoshida4(x,v,dtreal,method(:acc))
  else
    x,v = hybrid1(x,v,dtreal,method(:acc), kfunc, ifunc)
  end
  if (i+1)%ninter == 0
    printf("%25.20e %25.20e %25.20e %25.20e %25.20e %25.20e %d\n", (i+1)*dtreal,
